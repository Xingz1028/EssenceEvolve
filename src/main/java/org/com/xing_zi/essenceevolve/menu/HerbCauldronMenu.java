...
public class HerbCauldronMenu extends AbstractContainerMenu {
    ...
    private void addPlayerSlots(Inventory inventory) {
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 9; col++) {//Inventory里面前九个槽位是快捷栏位//每个格子是18*18，添加列
                this.addSlot(new Slot(inventory, col + row * 9 + 9, 8 + col * 18 + 24, 84 + row * 18));//添加行
            }
        }
    }
    ...
    private void addPlayerHotbarSlots(Inventory inventory) {
        for (int col = 0; col < 9; col++) {
            this.addSlot(new Slot(inventory, col, 8 + col * 18 + 24, 141 + 1));
        }
    }
