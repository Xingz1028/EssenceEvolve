package org.com.xing_zi.essenceevolve.entity.projectile.ball.EarthBallEntity;
...
public class EarthBallEntity extends ThrowableItemProjectile {
    private int tick;
    private final float DEFAULT_ATTACK_VALUE = 10F;
    private float attackValue = DEFAULT_ATTACK_VALUE;

    //存档、网络、summon指令 强制不能删
    public EarthBallEntity(EntityType<? extends ThrowableItemProjectile> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
    }

    //无射手，凭空坐标生成投射物
    public EarthBallEntity(Level pLevel) {
        super(EssEntityRegister.EARTH_BALL_ENTITY.get(), pLevel);
    }

    //玩家右键、怪物发射符咒时使用。
    public EarthBallEntity(LivingEntity pShooter, Level pLevel) {
        super(EssEntityRegister.EARTH_BALL_ENTITY.get(), pShooter, pLevel);
    }
    ...
    protected void onHitEntity(EntityHitResult pResult) {
        ...
        if (entity instanceof LivingEntity pLivingEntity) {
            ...
            pLivingEntity.addEffect(new MobEffectInstance(EssEffectRegister.EARTH_EFFECT.get(), 100, 0));
            ...
            for (LivingEntity ofClass : entitiesOfClass) {
                ofClass.addEffect(new MobEffectInstance(EssEffectRegister.EARTH_EFFECT.get(), 100, 0));
    ...
    protected void onHit(HitResult pResult) {
        ...
        if (!level.isClientSide()) {
            ...
            double dz = 0D;
            serverLevel.sendParticles(EssParticleRegister.WIND_FLY.get(), x, y, z, 1, dx, dy, dz, 0D);
            this.playSound(EssSoundRegister.EARTH_ATTACK.get());
            this.discard();
        }
    }
    ...
    public void tick() {
        ...
        if (tick % 3 == 0) {
            ...
            if (!level.isClientSide()) {
                ServerLevel serverLevel = (ServerLevel) level;
                for (int i = 0; i < 10; i++) {
                    double x = this.getX() + (level.random.nextDouble() - 0.5D);
                    double y = this.getY() + (level.random.nextDouble() - 0.5);
                    double z = this.getZ() + (level.random.nextDouble() - 0.5D);
                    double dx = 0D;
                    double dy = 0D;
                    double dz = 0D;
                    serverLevel.sendParticles(EssParticleRegister.LITTLE_SOIL.get(), x, y, z, 2, dx, dy, dz, 0D);
