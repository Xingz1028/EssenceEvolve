...
public class WaterBallEntity extends ThrowableItemProjectile {
    private int tick;
    private final float DEFAULT_ATTACK_VALUE = 10F;
    private float attackValue = DEFAULT_ATTACK_VALUE;
    //存档、网络、summon指令 强制不能删
    public WaterBallEntity(EntityType<? extends ThrowableItemProjectile> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
    }

    //无射手，凭空坐标生成投射物
    public WaterBallEntity(Level pLevel) {
        super(EssEntityRegister.WATER_BALL_ENTITY.get(), pLevel);
    }

    //玩家右键、怪物发射符咒时使用。
    public WaterBallEntity(LivingEntity pShooter, Level pLevel) {
        super(EssEntityRegister.WATER_BALL_ENTITY.get(), pShooter, pLevel);
    }

    @Override
    public void addAdditionalSaveData(CompoundTag pCompound) {
        super.addAdditionalSaveData(pCompound);
        pCompound.putFloat("essenceevolve_wand_attack_value",this.attackValue);
    }

    @Override
    public void readAdditionalSaveData(CompoundTag pCompound) {
        super.readAdditionalSaveData(pCompound);
        if (pCompound.contains("essenceevolve_wand_attack_value")){
            this.attackValue =  pCompound.getFloat("essenceevolve_wand_attack_value");
        }
    }
    ...
    protected void onHitEntity(EntityHitResult pResult) {
        ...
        if (entity instanceof LivingEntity pLivingEntity) {
            ...
            pLivingEntity.addEffect(new MobEffectInstance(EssEffectRegister.WATER_EFFECT.get(), 100, 0));
            ...
            for (LivingEntity ofClass : entitiesOfClass) {
                ofClass.addEffect(new MobEffectInstance(EssEffectRegister.WATER_EFFECT.get(), 100, 0));
                ofClass.hurt(ofClass.damageSources().thrown(this,this.getOwner()),attackValue*0.6F);
            }
            if (!level.isClientSide()) {
                ...
                for (int i = 0; i < 30; i++) {
                    ...
                    double dy = 0D;
                    double dz = 0D;
                    serverLevel.sendParticles(EssParticleRegister.WATER_TYPE_ONE.get(), x, y, z, 2, dx, dy, dz, 0D);
                    serverLevel.sendParticles(EssParticleRegister.WATER_TYPE_TWO.get(), x, y, z, 2, dx, dy, dz, 0D);
                    serverLevel.sendParticles(EssParticleRegister.WATER_TYPE_THREE.get(), x, y, z, 2, dx, dy, dz, 0D);
                    serverLevel.sendParticles(EssParticleRegister.WATER_TYPE_FOUR.get(), x, y, z, 2, dx, dy, dz, 0D);
                }
                for (int i = 0; i < 4; i++) {
    ...
    protected void onHit(HitResult pResult) {
        ...
        if (!level.isClientSide()) {
            ...
            double dz = 0D;
            serverLevel.sendParticles(EssParticleRegister.WIND_FLY.get(), x, y, z, 1, dx, dy, dz, 0D);
            this.playSound(SoundEvents.AMBIENT_UNDERWATER_EXIT);
            this.discard();
        }
    }
    ...
    public void tick() {
        ...
        this.setDeltaMovement(motion.x*0.97, motion.y+0.01, motion.z*0.97);
        if (tick % 3 == 0) {
            Level level = this.level();
            if (!level.isClientSide()) {
                ServerLevel serverLevel = (ServerLevel) level;
