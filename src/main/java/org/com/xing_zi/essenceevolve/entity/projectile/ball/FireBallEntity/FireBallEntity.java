package org.com.xing_zi.essenceevolve.entity.projectile.ball.FireBallEntity;
...
public class FireBallEntity extends ThrowableItemProjectile {
    ...
    public FireBallEntity(Level pLevel) {
        super(EssEntityRegister.FIRE_BALL_ENTITY.get(), pLevel);
    }

    //玩家右键、怪物发射符咒时使用。
    public FireBallEntity(LivingEntity pShooter, Level pLevel) {
        super(EssEntityRegister.FIRE_BALL_ENTITY.get(), pShooter, pLevel);
    }
    ...
    protected void onHitEntity(EntityHitResult pResult) {
        ...
        if (entity instanceof LivingEntity pLivingEntity) {
            ...
            for (LivingEntity ofClass : entitiesOfClass) {
                ofClass.addEffect(new MobEffectInstance(EssEffectRegister.FIRE_EFFECT.get(),100,0));
            ...
            if (!level.isClientSide()) {
                ...
                for (int i = 0; i < 30; i++) {
                    ...
                    double dy = 0D;
                    double dz = 0D;
                    serverLevel.sendParticles(ParticleTypes.SMALL_FLAME, x, y, z, 3, dx, dy, dz, 0D);
                }
                for (int i = 0; i < 4; i++) {
                    ...
                    double dy = 0D;
                    double dz = 0D;
                    serverLevel.sendParticles(EssParticleRegister.FIRE_EXPLOSION.get(), x, y, z, 1, dx, dy, dz, 0D);
                }
                }
    ...
    protected void onHit(HitResult pResult) {
        ...
        if (!level.isClientSide()) {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                double dy = 0D;
                double dz = 0D;
                serverLevel.sendParticles(ParticleTypes.SMALL_FLAME, x, y, z, 0, dx, dy, dz, 0D);
            }
                ...
                double dz = 0D;
                serverLevel.sendParticles(EssParticleRegister.FIRE_EXPLOSION.get(), x, y, z, 1, dx, dy, dz, 0D);
                this.playSound(EssSoundRegister.FIRE_ATTACK.get());
    ...
    public void tick() {
        ...
        if (tick % 3 == 0) {
            ...
            if (!level.isClientSide()) {
                ...
                double dy = 0D;
                double dz = 0D;
                serverLevel.sendParticles(ParticleTypes.SMALL_FLAME, x, y, z, 2, dx, dy, dz, 0D);
                serverLevel.sendParticles(ParticleTypes.SMOKE, x, y, z, 2, dx, dy, dz, 0D);
            }
        }
        if (tick % 5 == 0){
