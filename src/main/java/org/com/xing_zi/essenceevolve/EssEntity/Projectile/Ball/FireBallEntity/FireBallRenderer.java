...
public class FireBallRenderer extends EntityRenderer<FireBallEntity> {
    private final BallModel<FireBallEntity> model;

    public FireBallRenderer(EntityRendererProvider.Context pContext) {
        super(pContext);
        this.model = new BallModel<>(pContext.bakeLayer(EssModelLayerRegister.FIRE_BALL_LAYER));
    }
    @Override
    public void render(FireBallEntity fireBall, float yRot, float partialTick, PoseStack pose, MultiBufferSource buffer, int light) {
    ...
    public ResourceLocation getTextureLocation(FireBallEntity pEntity) {
        return ResourceLocation.fromNamespaceAndPath(Essenceevolve.MODID, "textures/entity/ball/fire_ball.png");
    }
}
